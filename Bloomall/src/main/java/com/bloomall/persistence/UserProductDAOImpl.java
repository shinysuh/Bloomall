package com.bloomall.persistence;

import java.util.List;
import java.util.Map;

import javax.inject.Inject;

import org.apache.ibatis.session.SqlSession;
import org.springframework.stereotype.Repository;

import com.bloomall.domain.CategoryVO;
import com.bloomall.domain.ProductVO;
import com.bloomall.util.SearchCriteria;

@Repository
public class UserProductDAOImpl implements UserProductDAO{

	@Inject
	private SqlSession session;
	
	private static final String NS = "com.bloomall.mappers.UserProductMapper";

	// 1차 카테고리 출력
	@Override
	public List<CategoryVO> primaryCtgrList() throws Exception {
		return session.selectList(NS + ".primaryCtgrList");
	}

	// 1차 카테고리에 따른 2차 카테고리
	@Override
	public List<CategoryVO> subCategoryList(String ctgr_cd) throws Exception {
		return session.selectList(NS + ".subCategoryList", ctgr_cd);
	}

	// 카테고리 이름 가져오기
	@Override
	public String getCtgrName(String ctgr_cd) throws Exception {
		return session.selectOne(NS + ".getCtgrName", ctgr_cd);
	}

	// 상품 리스트 (카테고리 선택 시)
	@Override
	public List<ProductVO> productList(Map<String, Object> map) throws Exception {
		return session.selectList(NS + ".productList", map);
	}
	
	//상품리스트(카테고리 - 1차)
	@Override
	public List<ProductVO> primeList(Map<String, Object> map) throws Exception {
		return session.selectList(NS + ".primeList", map);
	}

	// 상품리스트 ALL
	@Override
	public List<ProductVO> productListAll(Map<String, Object> map) throws Exception {
		return session.selectList(NS + ".productListAll", map);
	}

	// 상품 개수 (2차 카테고리 선택 시)
	@Override
	public int countByCtgr(String ctgr_cd) throws Exception {
		return session.selectOne(NS + ".countByCtgr", ctgr_cd);
	}

	// 상품 개수 (1차 카테고리 선택 시)
	@Override
	public int prime_countByCtgr(String ctgr_prt_cd) throws Exception {
		return session.selectOne(NS + ".prime_countByCtgr", ctgr_prt_cd);
	}

	// 상품 개수 (전체)
	@Override
	public int all_countByCtgr() throws Exception {
		return session.selectOne(NS + ".all_countByCtgr");
	}

	// 상품 리스트 (검색 시)
	@Override
	public List<ProductVO> productListSearch(SearchCriteria scri) throws Exception {
		return session.selectList(NS + ".productListSearch", scri);
	}

	// 상품 개수 (검색 시)
	@Override
	public int countBySearch(String keyword) throws Exception {
		return session.selectOne(NS + ".countBySearch", keyword);
	}

	// 상품 상세정보 페이지
	@Override
	public ProductVO productDetail(int prd_idx) throws Exception {
		return session.selectOne(NS + ".productDetail", prd_idx);
	}

	// 부모 카테고리 이름 가져오기
	@Override
	public String getPrtName(String ctgr_cd) throws Exception {
		return session.selectOne(NS + ".getPrtName", ctgr_cd);
	}

	
	
}
